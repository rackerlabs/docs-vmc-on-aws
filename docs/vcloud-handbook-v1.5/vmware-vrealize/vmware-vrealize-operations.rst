...
.. this file is used in two other documents: vcloud-handbook and
   vcloud-handbookv-1.5. It was impossible to single-source it with the current
   repository structure without loosing some of the content. If you are
   updating
   this file, you need to update this file in other locations as well.
...
.. _locating_ep_ops_agents-vcloud-book-v1.5:
...
.. note::
   ...
   refer to the `vRealize Operation Manager Installation and Configuration
   Guide
...
#. Locate the Linux (tar) EP Ops agent installation files (x32-bit or
   x64-bit) as described in :ref:`locating_ep_ops_agents-vcloud-book-v1.5`.
...
#. Run ``ep-agent.sh`` with ``start`` parameter and enter the correct
   ...
   .. code-block:: console
      ...
      Enter the server SSL port [default=443]: Press Enter to accept the
      default
